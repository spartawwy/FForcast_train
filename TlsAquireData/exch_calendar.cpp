...
T_TupleIndexLen ExchangeCalendar::GetStartIndexAndLen_backforward(int type_period, int start_date, int end_date)
{ 
    assert(trade_dates_->size() > 0);
    ...
    {
        if( type_period == TlsTypePeriod::PERIOD_5M )
        {
            lack_k_num = Get5mLackKnum(hhmm);
        }else if( type_period == TlsTypePeriod::PERIOD_1M )
        {
    ...
    switch( type_period )
    {
    case TlsTypePeriod::PERIOD_DAY: break;
